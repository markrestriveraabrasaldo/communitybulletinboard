-- Fixed Migration: Flexible Pricing System
-- Resolves column ambiguity issues
...
BEGIN
  ...
  FOR post_record IN 
    SELECT p.id, p.details, c.name as category_name
    FROM posts p
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE p.details ? 'pricing') as posts_with_new_pricing,
  COUNT(*) FILTER (WHERE p.details ? 'price') as posts_with_legacy_price,
  COUNT(*) FILTER (WHERE p.details ? 'price_range') as posts_with_legacy_price_range
FROM posts p
LEFT JOIN categories c ON p.category_id = c.id
GROUP BY c.name
...
SELECT 
  ...
  p.details->'pricing' as new_pricing_structure
FROM posts p
LEFT JOIN categories c ON p.category_id = c.id
WHERE p.details ? 'pricing'
